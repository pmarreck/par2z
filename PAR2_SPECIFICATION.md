...
## Language Recommendation for macOS Product
Primary recommendation: **Rust** with a C-ABI boundary for Swift integration.
- Pros: memory safety, strong performance, mature crates for hashing/CRC, easy to fuzz, good CI story.
- Cons: Swift interop adds build complexity, team familiarity may be lower, binary size can grow.

Alternative: **C++17** (or C) core with a thin Swift/Obj-C wrapper.
- Pros: maximal performance/control, straightforward FFI, mature RS implementations if you later need to compare behavior.
- Cons: manual memory safety, higher bug risk in parsing/RS math, more security review burden.

Alternative: **Swift** only (with Accelerate where possible).
- Pros: native macOS integration, simpler toolchain, faster product iteration.
- Cons: harder to hit peak performance on RS math, fewer low-level bit/field utilities, potential overhead for large sets.
