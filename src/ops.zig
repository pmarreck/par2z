...
pub fn create(opts: CreateOptions) !void {
	var arena = std.heap.ArenaAllocator.init(std.heap.page_allocator);
	defer arena.deinit();
	const allocator = arena.allocator();

	const inputs = try collectCreateInputs(allocator, opts.data_paths, opts.recurse, opts.basepath);
	...
	const creator_text = par2.version_string;
	...
	var main_file = try std.fs.cwd().createFile(opts.par2_path, .{ .truncate = true });
	...
	if (recovery_blocks > 0) {
		...
		if (plan.len <= 1) {
			...
			const thread_count = if (opts.memory_mb != null) 1 else @min(cpu, plan.len);
			if (thread_count <= 1) {
				for (plan) |vol| {
					try buildVolume(
						allocator,
